  ...
  useEffect(() => {
    const paymentDone = async () => {
      let res = await dispatch(
        CustomerActions.paymentDone({ payment_intent: id })
      );
      console.log(res, "response");
      if (res?.payload.success) {
        handleShowPreviousOrder();
        setOrderDate(
          moment(res?.payload?.data?.createdAt).format("DD MMM YYYY")
        );
        setOrderId(res?.payload?.data?.orderId);
        setOrderPrice(res?.payload?.data?.price);
        setOId(res?.payload?.data?._id);
